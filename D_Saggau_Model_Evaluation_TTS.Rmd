...
Kattan and Gerds (2018) suggest that the evaluation is somewhat problematic within a more practical setting. 
They argue that the measure is less interpretable, given that one is only able to compare the performance relative to the best useless model.
This implies that we are unable to see whether the implementation of the model is advisable in the first place.
...
One should note that as perhaps not obvious, clinicians don't value the different components of model accuracy eqally.
