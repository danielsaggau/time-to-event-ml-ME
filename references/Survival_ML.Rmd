...
Snippet Kattan and Gerds (2018) : 
### Accounting for time: Further Kattan and Gerds (2018) argue that model evaluation needs to account for the time horizon. 
...
* All in all, decision curve analysis is a complement not a substitue for AUC
* Interpretation requires understanding of the liking of the patient * 
* The proposed metho does not require obtaining information regarding treatment preferences but need theoretical relation for the threshold probabilty of diease and the realtive value of false positive and false negative results 
...
* By incorporating clinical consequences, s.a. benfit of finding disease early or the harm of unnecessary further testing 
* Net benefit is simply type: we rescale harm by exchange rate to put on same scale as benefit 
* plotting net benfit against range of exchange rates in what is called a decision curve 
* This exchange rate can be dervied by asking questions such as max. number of patient a doctor would recommend for biopsy to find one cancer. (subjective)
...
* Prognostic models account for dimenison of time and introduce a stochastic element 
...
* Prognostic studies used for risk stratification for assessing the level of risk 
*Example: Framingham risk score for cardiovascular disease
*Screen for early detection can be done for diagnostic such as cancer screening or colonsoscopy 
Screening results can subsequently be used in prognostic models for later events 
* discrimination and calibration both important for prognostic studies
* Area under the curve is also known as he c-statistic or the c index and can rage fromm 0.5 (no predictive ability, bascially random) or 1 (perfecr discrimination)
* c statistic is based on rank of predicted probabilites and compares ranks in indivudals with and without disease 
* Similar to wilcoxon rank sum statistic (see 9) 
* Cam be computed parametic and nonparametic
* The ROC curve and c statistic are insenstive in assessing the impact of adding new predictiors to a score or predictive model (see 14)
* The change in the ROC curve depends on both the predictive ability of the orginal set and the strength of the new marker , as well as the correlation between them 
* For calibration, the most popular measure is the hosmer lemeshow goodness of fi test (16) 
* Problem here: sensitive to how he subgroups are formed but groups need to be formed for evaluation (17)
* When predicting true probability, perfect calibration
* Clinical reclassification measures only discrimination
* Advantage over ROC is that categories based on clinical important risk estimations 
* For clinical studies ofen the intermediate risk categories are most important where treatment is questionable 
* In summary, NRI and calibration test for cross-classified categories can be used to foramlly assessed clinical utility


### Wu, Y. C., & Lee, W. C. (2014). Alternative performance measures for prediction models. PLoS One, 9(3), e91249.

* sbrier upon addition of new markers i equal to the IDI index 
* Gini, pietra nd sbrier can be expressed as ration, comparing the resolution power 
