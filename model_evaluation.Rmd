...
For every TTE study, we have a hazard function
...
For further information, Blanche et al. (2019) explicitly elaborate why one cannot use the c-index for t-year predictions.
Their arguments boil down to the following mechanism, namely that with concordance scores we are comparing actual event times as compared to the (time dependent) AUC which compares binary event status at time t.
@blanche_c-index_2019
...
Where L is the loss function, S is the survival function, G is IPCW based estimated event, t* is the .
