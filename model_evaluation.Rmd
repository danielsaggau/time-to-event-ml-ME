...
This feature in our data makes it challenging to use classical model evaluation tools and require some modifcations.
...
The section thereafter discusses of these methods, also briefly followed by a short discourse to novel model evaluation tools focusing on clinical usefulness.
Here, the focus is on decision curve analysis and Net Reclassification improvement.
...
Two popular hazard functions are the proportional hazard model e.g. the Cox-PH-Model and the non-parametric Hazard model such as the Kaplan-Meier Model (Gerds et al., 2006)
...
On the one hand we have right censored data, data where e.g. the event did not take place yet and the real event takes place in the future.
On the other hand we have left censored data, data where e.g. the event took place at a point between our specified time thresholds, not allowing us to record the exact time accordingly and thus the real event is in the past.  (Steyerberg et al., 2010; Fu & Simonoff, 2016)
...
Therefore, we are testing how strong our model discriminates between subjects that incur an event versus subjects that don't. 
...
The most prominent score to capture accuracy in classification tasks is the brier score, a modification of the mean squared error (Assel et al., 2017; Gerds et al., 2006).
\newline 
...
To follow understand where these methods, the focus will be what differentiates these methods from classical tools.
...
## C-Index: Adjustments for censored data
...
Therefore, we can firstly use a concordance statistics rather than an AUC, to accommodate some of the data.
...
In a clinical setting rules of thumb such as a score of ~ 0.8 good will not always hold. (Cook, 2007)
A concordance pair is a pair that is consistent, henceforth subjects with higher risks have earlier events and subjects with lower risk scores translate in later event time points. (Antolini et al., 2005)
...
Packages such as 'pec' use a time dependent c-index proposed by Antolini et al. (2005).
...
Essentially, we are comparing actual and predicted scores of y and square their difference, giving each observation equal weight.
...
where L is the loss function, S is the survival function, G is IPCW estimate of the censored survival function $P(C*>t)$, N is the number of observations , $S_i$ is the predicted survival function, t is the time of the event, $t^*$ the time before event and $\delta$ is our indicator whether our data is censored or not.
For the population score, the following notation is specified: 

$$
L(S) = \frac{1}{NT}\displaystyle\sum^N_{i=1}\displaystyle\sum ^T_{j=1}L(S_i,t_i|t^*) \tag{9}
$$

Note that the method how the scores are integrated differs per package.
E.g. in mlr3proba you can integrate via equal weights (method==1), or use difference between time points (method==2).
...
Here, we are using the default method,the Kaplan-Meier estimates, for the IPCW (Gerds, 2020).
...
As a reminder, we are interest in the different thresholds and not only want a single score.
...
## Novel Research: Net Reclassification improvement
...
## Novel Research: Decision Analysis Curve 
