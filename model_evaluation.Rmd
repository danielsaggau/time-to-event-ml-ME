...
We don't always have full information on the events for all subjects in our dataset. 
This is a common challenge of Time-to-Event studies, and referred to as censored data.
Reasons for having censored data in our dataset can be manifold.
...
Both methods use the inverse probability of the censoring weighted estimate (IPCW) for the censored data.
The c-index does enjoy prominence due to interpretability and the ability to make insightful conclusions for individual subjects.
...
# Fundamental Ideas in Time-to-Event Studies 
...
**Calibration**: Calibration deals with our predictive accuracy.
...
The concordance- index is the generalization of the ROC for survival data (Cook, N., 2007).
...
On the other hand, we would specify the C score as follows:
...
Here, we are using the default method,the Kaplan-Meier estimates, for the IPCW.
\newline
\footnotesize
```{r}
perror <- pec(
...
(2) Prognostic studies need to account for the model accuracy which is measured by model calibration.
...
(4) Because c-statistics is based on ranks it is less sensitive than e.g. measures based on probabilities.
...
(3) Moreover, the c-index does not allow for t-year predictions. 
For the IBS, we deal estimates specifically for a time-specified horizon.(Kattan and Gerds, 2018)

**Disadvantages**:(1) Kattan and Gerds (2018) suggest that the evaluation is somewhat problematic.
...
(2) On the one hand, scores are affected by overall event risk.
...
(3) Clinicians usually don't value the different components of model evaluation equally as their clinical consequences are not equivalent.
...
Integrated discrimination improvement (IDI) is equivalent to testing whether the regression coefficient in a model is equal to zero (Cook & Ridker , 2009).
