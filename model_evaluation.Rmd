...
One does not always have full information on the time when events occur for every subjects in our data. 
...
Having censored data makes it challenging to use classical model evaluation tools and requires some modifications.
The most popular tools are the mean squared error (MSE) and the area under the curve (AUC), and respective extensions are the the Integrated Brier Score (IBS) and the concordance-index or in short c-index.
...
Subsequently, the area under the curve integrates over all possible mis-classification rates.
...
We are interest in the different thresholds and not only want a single score, hence the boxplot representation is suitable for visual representation.
...
Harm is transformed, using an exchange rate to put harm and benefit on one scale (Vickers et al., 2006).
