...
models <-  list("Cox.X1"=coxph(Surv(time,status)~X1,
               ...
               "Cox.X1.X2"=coxph(Surv(time,status)~X1+X2,
                      data=dat,x=TRUE,y=TRUE))
# prediction error based on brier score
perror <- pec(object=models,
                 formula=Surv(time,status)~1,  # KM covariate specification; For Cox we use e.g.~ X1 + X2.
              data=dat, 
                 exact=TRUE, cens.model="marginal", # Estimator for IPWC ; For Cox we would use  cens.model="cox"
              splitMethod="none", 
...
```{r}
summary(perror,times= quantile(dat$time[dat$status==1], c(.25, .5, .75)))
```

## Plotting prediction error

```{r}
plot(perror,xlim=c(0,25), ylim = c(0,0.5))
```
...
cindex = cindex(models, formula = Surv(time,status) ~ 1,
     cens.model="marginal", data = dat,
      eval.times= quantile(dat$time[dat$status==1], c(.25, .5, .75)))
```

## c-index plot

```{r}
plot(cindex,xlim=c(0,25), ylim =c(0.4,1))
```
